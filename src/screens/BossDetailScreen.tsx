...
export function BossDetailScreen({ route }: any) {
  ...
  const attrs = getBossAttributes(boss);
  ...
  return (
    ...
    >
      ...
      <Text style={styles.paragraph}>{attrs.weaknesses.join(", ") || "—"}</Text>
      <Text style={styles.sectionTitle}>Resistance:</Text>
      <Text style={styles.location}>{attrs.resistances.join(", ") || "—"}</Text>

      <Text style={styles.sectionTitle}>Recommended teams</Text>
      {teamsMatchingWeakness(attrs.weaknesses).map((team) => (
